...
struct ScreenLastik: View {
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 10) {
                    ...
                    .cornerRadius(10)
                    .padding(.bottom)
                    .onChange(of: secilenIslem){ _,_ in
                    ...
                    if secilenIslem == "Yeni Lastik" {
                        ...
                        .cornerRadius(10)
                        .padding(.bottom)
                        .onChange(of: secilenInput){ _,_ in
                            deleteModelsOtomobil()
                            deleteModelsSuv()
                            deleteModelsMoto()
                            deleteLastik()
                        ...
                        if secilenInput == "Ebata Göre" {
                            ...
                            customPicker("Mevsim", selection: $secilenMevsim, options: mevsimler)
                                ...
                                .onChange(of: secilenMevsim){ _,_ in
                    ...
                    Button(action: {
                        ...
                        if secilenIslem == "Yeni Lastik" {
                            if secilenInput == "Ebata Göre" {
                                kaydetLastik(kategori: "Lastik Değişim",altKategori: "Ebata Göre", marka: "", model: "", yil: "", genislik: secilenGenislik!, oran: secilenOran!, cap: secilenCap!, mevsim: secilenMevsim!, longituteAnlik: location.longitude, latitudeAnlik: location.latitude) { result in
                                    switch result {
                                ...
                                switch secilenKategori {
                                case .otomobil:
                                    kaydetLastik(kategori: "Lastik Değişim",altKategori: " Modele Göre", marka: secilenMarkaOtomobil!.rawValue, model: secilenModelOtomobilDeneme, yil: secilenYil!, genislik: "", oran: "", cap: "", mevsim: "", longituteAnlik: location.longitude, latitudeAnlik: location.latitude) { result in
                                        switch result {
                                        case .failure(let error):
                                            print(error.localizedDescription)
                                        case .success(_):
                                ...
                                case .motor:
                                    kaydetLastik(kategori: "Lastik Değişim",altKategori: " Modele Göre", marka: secilenMarkaMotor!.rawValue, model: secilenModelMotorDeneme, yil: secilenYil!, genislik: "", oran: "", cap: "", mevsim: "", longituteAnlik: location.longitude, latitudeAnlik: location.latitude) { result in
                                        switch result {
                                        case .failure(let error):
                                            print(error.localizedDescription)
                                        case .success(_):
                                ...
                                case .suv:
                                    kaydetLastik(kategori: "Lastik Değişim",altKategori: " Modele Göre", marka: secilenMarkaSuv!.rawValue, model: secilenModelSuvDeneme, yil: secilenYil!, genislik: "", oran: "", cap: "", mevsim: "", longituteAnlik: location.longitude, latitudeAnlik: location.latitude) { result in
                                        switch result {
                                        case .failure(let error):
                                            print(error.localizedDescription)
                                        case .success(_):
                            ...
                            //kullanıcı adı soyad konumu
                            kaydetLastik(kategori: "Tamir-Stepne Değişim",altKategori: secilenIslem, marka: "", model: "", yil: "", genislik: "", oran: "", cap: "", mevsim: "", longituteAnlik: location.longitude, latitudeAnlik: location.latitude) { result in
                                switch result {
                                case .failure(let error):
                                    print(error.localizedDescription)
                                case .success(_):
                    ...
                    .disabled(
                        (secilenYil == nil) && (secilenMevsim == nil)  && (secilenIslem == "Yeni Lastik")
                    )
                    .opacity(
                        ((secilenYil == nil) && secilenMevsim == nil && (secilenIslem == "Yeni Lastik")) ? 0.5 : 1.0
                    )
    ...
    func kaydetLastik(
        ...
        cap: String,
        mevsim: String,
        longituteAnlik: Double,
        latitudeAnlik: Double,
        completion: @escaping (Result<VasitaParseLastik, Error>) -> Void
    ) {
        var lastik = VasitaParseLastik()
        lastik.kategori = secilenIslem
        lastik.marka = marka
        lastik.model = model
        lastik.altKategori = altKategori
        lastik.yil = yil
        lastik.genislik = genislik
        lastik.oran = oran
        lastik.cap = cap
        lastik.mevsim = mevsim
        lastik.longituteAnlik = longituteAnlik
        lastik.latitudeAnlik = latitudeAnlik
        lastik.fullName = User.current?.fullName
        lastik.kullanici = User.current
        lastik.emailKullanici = User.current?.email
        lastik.save { result in
            switch result {
            case .success(let savedLastik):
                print("Başarıyla kayıt edildi: \(savedLastik)")
                completion(.success(savedLastik))
            case .failure(let error):
